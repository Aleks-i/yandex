...
public class PathInGraph {
    public static void main(String[] args) throws IOException {
        ...
        findShortestWay(graph, vStart, queue, way, prev);
//        printGraphs(graph);
        if (vStart == vTarget) {
            ...
            List<Integer> wayStorage = new ArrayList<>();
            int v = vTarget;
            while (v != 0){
                wayStorage.add(v);
                v = prev[v];
            }
            Collections.reverse(wayStorage);
            wayStorage.forEach(w -> System.out.print(w + " "));
        }
    }

    private static void findShortestWay(List<List<Integer>> graph, int vStart, LinkedList<Integer> queue, int[] way, int[] prev) {
